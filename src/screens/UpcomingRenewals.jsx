...
import { selectAdminLoginUserDetails } from "../statemanager/slices/AdminLoginUserSlice";
...
const AdminUpcomingRenewals = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // alert("upc" + LoginDetails?.AccountId);
        const policies = [];
    ...
    >
      ...
      >
        {allPolicies.length === 0 ? (
          <h3>No upcoming renewals withing the selected period</h3>
        ) : (
          <table>
            <thead>
              {" "}
