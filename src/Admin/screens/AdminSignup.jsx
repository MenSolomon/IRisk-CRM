...
import RowRadioButtonsGroup from "../components/RadioGroup/BasicRadio";
import {
  setAdminLoginStatus,
...
const AdminSignup = () => {
  ...
  const [adminPrivilege, setAdminPrivilege] = useState("Read");

  // function to create a new user
  const onSubmit = async (formData, e) => {
    e.preventDefault();

    const { FirstName, LastName, EmployeeID, Email, Password } = formData;
    ...
    try {
      ...
      await createUserWithEmailAndPassword(auth, Email, Password)
        .then(async (userCredential) => {
          ...
          const adminRef = doc(db, `admins_db`, user.uid);
  ...
  return (
    <div className={signupcss.container}>
      <div style={{ background: "#8b3cad" }} className={signupcss.leftdisplay}>
      ...
      <div className={signupcss.rightdisplay}>
        ...
        <div>
          ...
          <span style={{ color: "blue", cursor: "pointer" }}>
            {" "}
            <Link to="/Admin_login"> log in to your account</Link>
          </span>{" "}
        </div>{" "}
          ...
          <ul>
            ...
            <li>
              <RowRadioButtonsGroup
                ...
                radioArray={["Read", "Read Write", "Full Access"]}
                onChange={(e) => {
                  setAdminPrivilege(e);
                }}
                defaultValue={adminPrivilege}
              />
            </li>
          </ul>
          <Button
